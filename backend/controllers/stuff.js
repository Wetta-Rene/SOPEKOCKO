...
exports.likeOrDislike = (req, res, next) => {
    if(req.body.like === 1){// utilisateur aime la sauce
        ...
        .then ((sauce)=> res.status(200).json({ message: '1-Like modifié !'}))
        .catch(error => res.status(400).json({ error }));
    } else if (req.body.like === -1){
        Sauce.updateOne({ _id: req.params.id },  {$inc: {dislikes: (req.body.like++)*-1} ,$push: {usersDisliked: req.body.userId}})
        .then ((sauce)=> res.status(200).json({ message: '2-Like modifié !'}))
        .catch(error => res.status(400).json({ error }));
    } else{ //like vaut 0
        if(Sauce.find({usersLiked: req.body.userId})){
        Sauce.updateOne({_id: req.params.id}, {$pull: {usersLiked: req.body.userId}, $inc: {likes: -1}})
             .then((sauce) => {res.status(200).json({ message: '3-Like modifié !'})}) 
            .catch(error => res.status(400).json({ error }));
        }else{
         Sauce.updateOne({_id: req.params.id}, {$pull: {usersDisliked: req.body.userId}, $inc: {dislikes: -1}})
            .then((sauce) => {res.status(200).json({ message: '4-Like modifié !'})}) 
            .catch(error => res.status(400).json({ error }));           
        }
    }
};
